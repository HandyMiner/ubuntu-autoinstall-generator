...
cd "$tmpdir"
mkisofs -quiet -D -r -V "ubuntu-autoinstall-$today" -cache-inodes -J -l -joliet-long -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -eltorito-platform 0xEF -eltorito-boot boot/grub/efi.img -no-emul-boot -o "${destination_iso}" .
cd "$OLDPWD"
